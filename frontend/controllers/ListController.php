...
/**
 * ListController process the actions specific to Mailing List model.
 *
 * @since 1.0.0
 */
class ListController extends \cmsgears\newsletter\frontend\controllers\base\Controller {
	...
	public function actionUnsubscribe( $id, $mgid ) {
		...
		if( isset( $model ) && isset( $member ) && $model->memberId == $member->id ) {

			// Match Member GID
			if( $member->load( Yii::$app->request->post(), $member->getClassName() ) && $member->validate() ) {

				$this->modelService->disable( $model );

				// Set Flash Message
				Yii::$app->session->setFlash( CoreGlobal::FLASH_GENERIC, Yii::$app->newsletterMessage->getMessage( NewsletterGlobal::MESSAGE_UNSUBSCRIBE_LIST ) );
